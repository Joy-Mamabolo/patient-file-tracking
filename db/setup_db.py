...
def setup_database():
    with app.app_context():
        ...
        if not models.PatientFile.query.first():
            db.session.add(models.PatientFile(patient_name = "Emily Davis", dob = datetime.date(1985,4,12), status_id = 1))
            db.session.add(models.PatientFile(patient_name = "Michael Wilson", dob = datetime.date(1990,9,23), status_id = 2))
            db.session.add(models.PatientFile(patient_name = "Sarah Miller", dob = datetime.date(1978,11,5), status_id = 3))
            db.session.add(models.PatientFile(patient_name = "David Anderson", dob = datetime.date(1965,2,17), status_id = 1))
            db.session.add(models.PatientFile(patient_name = "Laura Thomas", dob = datetime.date(2000,7,30), status_id = 2))
            db.session.commit()
